...
""", language="python")
st.markdown("""
> 使用过去 200 个交易日（约 10 个月）的最高**收盘价**作为突破基准，突破窗口与策略1.0相同。
> 注意：v2 使用**收盘价**衡量历史高点（v1 使用最高价），门槛略严格，需回测验证差异。
""")
...
_diff_rows = [
    ("入场", "突破窗口 N", "200 日最高价（high）", "200 日最高收盘价（close）", "窗口相同；v2 用收盘价（v1 用最高价），门槛略严格"),
