...
| 止损 | ATR 止损 | **结构止损（ATR + 近期低点双重保护）** |
| 止盈 | 分段移动止盈（3/3/5×ATR） | 分段移动止盈（3/3/5×ATR）＋平价保护（⚠️ 设计选项，待回测验证） |
...
| 止损类型 | 策略1.0 | 策略2.0 |
|---------|---------|---------|
| 方法 | ATR 止损（`entry - 2×ATR`） | **结构止损**（ATR 止损与近期低点止损取较高值） |
...
if low[t] < stop_loss:
    ...
    exit at open[t+1]       # 移动止盈触发（收盘价）
""", language="python")

st.markdown("""
与策略1.0使用**完全相同**的分段移动止盈机制：
...
开仓初期，移动止盈初始值（entry − 3×ATR）低于止损（entry − 2×ATR），故止损先提供保护；
...
2. **与移动止盈逻辑矛盾**：当浮盈正好达到 1R 时，trail_stop 仍低于入场价（entry − ATR），
   说明移动止盈认为此时距离合理噪音范围还有空间。平价保护强行插入更紧的约束，
   与"给趋势充分发展空间"的设计初衷冲突。
...
_param_rows = [
    ...
    ("Exit",  "移动止盈早期倍数（R<1）", "3×ATR", "2× / 3× / 4×", "早期止损松紧"),
    ("Exit",  "移动止盈后期倍数（R≥3）", "5×ATR", "4× / 5× / 6×", "大趋势阶段止损宽松程度"),
...
_diff_rows = [
    ...
    ("止损", "机制", "ATR 止损（entry - 2×ATR）", "结构止损（max(ATR止损, 近期低点×0.995)）", "2.0 结合价格结构"),
