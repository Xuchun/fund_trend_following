...
render_page_header("参数敏感性分析(Tiingo)", meta)
...
确保持仓能在合理冲击成本下退出。\$50M 是当前 Tiingo Baseline 基准，
实盘容量 \$1000 万时，\$50M ADV 意味着典型仓位（约 \$50 万）仅占当日成交量约 1%，冲击成本极低。""",
    )
    base = _info["base_rec"]
    best = _info["best_sharpe_rec"]
    lo, hi = _info["stab_lo"], _info["stab_hi"]
...
""")
    st.markdown(
        ...
        "$50M ADV 意味着占当日成交量约 1%，冲击成本极低，可轻松完成建仓退出；"
        "相比 $20M 门槛，$50M 更贴近实盘大资金的流动性需求，同时标的池仍覆盖 2,937 只历史标的"
    )
