...
st.subheader("1. 标的过滤")
st.markdown(f"""
标的池来自 **Tiingo EOD 数据**（NYSE / NASDAQ / AMEX 全量历史股票 + 87 只 ETF，含退市标的），
采用两级过滤机制（详见「数据与标的池」页面）：

**静态预过滤（CSV 构建时）：** 仅纳入在价格 > \${p.get('min_price', 10):.0f} 且 ADV > 阈值的条件下，累计满足天数 ≥ 252 天（约 1 年）的标的，确保引擎有足够的历史数据计算 200 日突破信号。
...
实盘中几乎从不触发——3,053 笔历史交易中无一笔止损距离低于 0.5%，
