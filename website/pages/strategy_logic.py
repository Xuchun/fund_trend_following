...
- 与移动止盈（用收盘价触发）不同：硬止损更激进，能在当日即捕捉到价格大幅下破
""")
...
# ── Trailing stop ─────────────────────────────────────────────────────────────
st.subheader("7. 分段移动止盈")
st.markdown(f"""
随着持仓盈利增加，逐步收紧移动止盈以锁定利润：
...
其中 `highest_high` = 持仓期间（自入场日起）adj_high 的累计最大值；**初始化为 `entry_price`**（不使用入场日当天盘中最高价，避免引入未来信息）。止损线锚定最高点而非当日收盘价，确保止损线仅在创新高时上移，不会因某天大涨后小幅回调就提前触发。

**初始值与硬止损的切换逻辑：**

```
开仓时：trail_stop[t0] = entry_price − 3×ATR   （移动止盈初始值，宽于硬止损）
硬止损：stop_loss      = entry_price − 2×ATR   （固定，日内最低价触发）
```

开仓初期，硬止损（2×ATR）比移动止盈（3×ATR）更紧，由硬止损提供主要保护。
当持仓最高价突破 `entry_price + 1×ATR` 后，移动止盈线（`highest_high − 3×ATR`）
开始高于硬止损线（`entry − 2×ATR`），从此由移动止盈主导退出。
...
两者同时有效，以**收盘价穿越移动止盈**或**日内最低价穿越硬止损**中先触发者为准。
""")
...
| Priority 2 | 移动止盈：`close[t] < trail_stop` | 收盘价，仅在硬止损未触发时检查 |
