...
st.markdown("""
| 模块 | 主要改进建议 | 有效性 | 精准度 | 年化收益 | 最大回撤 | 实现难度 | 优先级 |
|------|------------|:-----:|:-----:|:-------:|:-------:|:-------:|:-----:|
| **1. 标的过滤** | ADV门槛通胀动态调整 | ✅ 提升 | 中性 | +0.5–1% | 中性 | ★★ 中 | 🟡 |
| **2. 市场环境过滤** | 增加 Choppiness Index 二层过滤 | ✅ 提升 | 中性 | 中性 | ✅ 降低 | ★★ 中 | 🟡 |
| **3. 成交量过滤** | Walk-Forward 验证 1.5× 阈值 | ✅ 提升 | 中性 | 中性 | 中性 | ★★ 中 | 🟡 |
| **4. 入场条件** | **金字塔加仓**；Gap 改为 ATR 基准 | ✅ 提升 | ✅ 提升 | **+4–5%** | -1–2% | ★★★ 高 | 🔴 **最高** |
| **5. 初始止损** | 止损执行价改为 min(stop, open[t+1]) | 中性 | ✅ **最高** | -0.74pp | 中性 | ★ 低 | 🔴 **高** |
| **6. 分段移动止盈** | ≥10R 新增第四档 7×ATR | 中性 | 中性 | +1–2% | 中性 | ★ 低 | 🔴 高 |
| **7. 仓位管理** | 热度上限 heat_limit 降至 8% | 中性 | 中性 | -0.3% | ✅ 降低 | ★ 低 | 🟡 |
| **8. 执行假设** | 分层滑点；止损执行价（同第5节） | ✅ 提升 | ✅ 提升 | 中性 | 中性 | ★ 低 | 🟡 |

> **图例：** ✅ 改善 / 中性 / -降低。优先级 🔴 = 高影响，🟡 = 中等影响。
""")
