...
st.markdown("#### C. 三层验证体系")

st.markdown("""
不同类型的改动需要不同深度的验证。验证层级越高，对结论的信心越强。
""")

_val_data = {
    "验证层级": ["Layer 1：IS 回测 + 扰动分析", "Layer 2：Walk-Forward OOS 验证", "Layer 3：模拟交易验证"],
    "核心问题": [
        "改动在全历史数据上是否改善了表现？响应曲线是否平滑稳健？",
        "改动在未参与训练的样本外数据（OOS）上是否仍然有效？",
        "改动在真实市场条件下（非历史数据）是否有效？",
    ],
    "关键判断标准": [
        "指标改善 + 扰动曲线单调或平滑（无孤峰）",
        "OOS 效率比 ≥ 0.5（OOS CAGR / IS CAGR）",
        "模拟交易 6–12 个月内与 IS 预期行为一致",
    ],
    "适用的改动类型": [
        "参数微调（止损乘数、窗口期等）",
...
st.dataframe(
    __import__("pandas").DataFrame(_val_data),
    use_container_width=True, hide_index=True,
)

st.info("""
**Walk-Forward OOS 效率比解读（本策略参考值）：**

OOS 效率比 = OOS 年化收益 / IS 年化收益。本策略当前 Walk-Forward 结果约为 0.6–0.8，
属于可接受范围（> 0.5）。**任何改进必须在 Walk-Forward 后 OOS 效率比不低于改进前**，
否则说明改动虽然改善了 IS 数据的拟合，但降低了策略的泛化能力——即过拟合。
""")

st.markdown("#### D. 针对不同改动类型的具体操作规范")
...
- 扰动分析中，该参数的响应曲线在最优点附近**平滑**（非孤峰），说明结论对参数值不敏感
- 交易笔数变化不超过 ±15%（过多/过少说明参数拉动了过滤器行为）

**反例（不应接受）：**
停损乘数从 2× 改为 2.03× 使 CAGR 从 10.58% 提升至 10.62%——
改善极小，且 2.03 这个值没有任何经济学依据，纯属拟合。
...
with st.expander("🆕 新增过滤逻辑（如 Choppiness Index、时间止损、Gap 过滤改 ATR 基准）"):
    st.markdown("""
**验证要求：Layer 1 + Layer 2（IS + Walk-Forward）**
...
4. 计算 OOS 效率比，要求 ≥ 0.5，且 OOS 期间改进方向与 IS 一致
...
with st.expander("🏗️ 架构级改动（金字塔加仓、多空切换对冲、Cluster Risk 组合级上限）"):
    st.markdown("""
**验证要求：Layer 1 + Layer 2 + Layer 3（IS + Walk-Forward + 模拟交易）**
...
- **模拟交易是最后防线**：IS + WF 都通过的改动，仍需在真实市场中跑 6–12 个月，
...
✅ **扰动分析验证参数稳健性**
- 13 个核心参数的扰动结果（见本页各节）显示响应曲线大多平滑，
  证明策略性能对参数值不过度敏感

✅ **Walk-Forward 建立样本外防线**
- 本策略已有完整的 WF 验证框架（见「Walk-Forward 验证」页面）
""")
with _col_e2:
    st.markdown("""
**仍需注意的风险：**
...
建议为每个重大改动专门预留一段时间段作为最终验证集（如 2023–2026 年），
在所有 IS 分析结束后，用这段时间做一次性最终验证。

⚠️ **模拟交易数据尚短**
当前模拟交易仅运行数周，尚无法提供 Layer 3 验证。需要至少 6 个月的模拟交易数据，
