...
# ── 一、方法论：防过拟合实验框架 ─────────────────────────────────────────────────
st.subheader("一、方法论：如何做实验和分析，才不导致过度拟合")
st.markdown("在动手改进策略之前，先建立一套让结论可信的实验规范")

st.markdown("""
**核心困境**：回测永远在已知结果的历史数据上运行。研究者可以（无意识地）不断调整参数和逻辑，
直到历史表现满意——但这个过程实际上是在让参数「记忆」历史噪音，而非学习真正的规律。
结果是：回测优秀，实盘失效。
""")

st.markdown("#### A. 过拟合是如何发生的")
...
1. **自由度过多**：策略有 12 个参数 + 数十种潜在改进，历史数据 26 年看似很长，
   但如果每次改进都在全量数据上验证，等于反复「消费」同一段历史，自由度累积极快。

2. **选择性报告**：只展示有效的改进，不展示失败的实验。
   若测试了 10 种信号过滤器，只有 2 种历史表现改善，却只呈现这 2 种——
   这等同于多重比较问题（Multiple Comparison），2 种可能完全是随机偶合。

3. **结果驱动设计**：知道哪些年份策略亏损（如 2015–2016 年），
   然后设计一个「刚好能过滤掉这段时期信号」的逻辑——这是最危险的过拟合形式，
   因为逻辑看起来有经济学依据，实则只是事后解释。
""")
with _col_a2:
    st.markdown("""
**过拟合的典型症状：**
...
| 症状 | 说明 |
|------|------|
| IS 改善，OOS 无改善 | 改进只对训练数据有效 |
| 只在某段时期有效 | 参数记住了特定市场状态 |
| 改动「刚好」修复了已知亏损期 | 结果驱动设计 |
| 参数越来越多 | 自由度持续累积 |
| 响应曲线有「孤峰」 | 最优点极度敏感，不稳健 |
| 改进需要精确到小数点后两位 | 正常市场规律不需要如此精确 |

**一个直觉类比**：给定足够多参数，任何模型都可以完美拟合历史数据——
就像用多项式可以穿过任意 n 个点。问题不是能不能拟合，而是拟合的是规律还是噪音。
""")

st.markdown("#### B. 三条铁律：实验的基本纪律")
...
例如：「我认为将止损从 2× ATR 改为 2.5× ATR 会降低最大回撤，
原因是更宽的止损可以让趋势有更多呼吸空间，减少被噪音震出的概率。」

**为什么有效**：强迫思考改进的经济学依据，
而非事后从结果反推原因。
若假设不成立，应诚实承认，而非继续调参直到「符合」假设。
""")
with _col_b2:
    st.markdown("""
**铁律二：一次只改一个变量**

若同时修改止损乘数 + 成交量过滤 + 移动止盈，
无法判断是哪个改动带来了改善（或恶化）。

**正确做法**：
1. 只改止损乘数 → 记录结果
2. 恢复原参数，只改成交量过滤 → 记录结果
3. 再考虑两者叠加是否有协同效应

**例外**：当两项改动在逻辑上不可分离时
（如金字塔加仓必须同时调整 position_cap），
需作为一个整体测试，并明确说明两者的耦合关系。
""")
with _col_b3:
    st.markdown("""
**铁律三：多指标评估，不只看 CAGR**

一项改动「有效」的判定标准不能只是 CAGR 提高。
需要同时看：
...
若 CAGR 上升但 Sharpe 下降，说明改进只是加了风险，
而非提升了策略本身的质量。
""")

st.markdown("#### C. Research Pipeline：四层验证，缺一不可")

st.markdown("""
验证层级不是"参考项"，而是**过滤器**。任何改进只要在某一层未能通过，应立即停止，
不应带着"先上线再说"的侥幸心理继续推进。
""")

st.code("""
提出假设（先写清楚经济学逻辑，再跑回测）
        ↓
Layer 1：IS 全样本 + 扰动分析（高原 vs 孤峰？）
        ❌ 无改善 或 响应曲线出现孤峰 → 停止
        ✅ 通过 → 继续
        ↓
Layer 2：Walk-Forward OOS 验证 + 多市场环境检验
        ❌ OOS 效率比下降 或 只在 1–2 个市场环境有效 → 停止
        ✅ 通过 → 继续
        ↓
Layer 3：蒙特卡洛鲁棒性验证
        ❌ 负收益概率显著上升 或 最差路径大幅恶化 → 重新审视
        ✅ 通过 → 继续
        ↓
Layer 4：模拟交易验证（终极检验，6–12 个月）
        ❌ 实盘行为偏离 IS 预期 → 不上线
        ✅ 通过 → 正式进入策略
""", language="")
...
_val_data = {
    "验证层级": [
        "Layer 1：IS 全样本 + 扰动分析",
        "Layer 2：Walk-Forward OOS + 市场环境",
        "Layer 3：蒙特卡洛鲁棒性验证",
        "Layer 4：模拟交易验证",
    ],
    "核心问题": [
        "IS 上改善吗？参数响应曲线是高原还是孤峰？",
        "OOS 上是否仍然有效？是否只在特定市场环境奏效？",
        "随机路径下结果是否稳健？最差情形是否可接受？",
        "真实市场中行为是否符合回测预期？",
    ],
    "关键判断标准": [
        "指标改善 + 扰动曲线平滑（高原形态，最优点两侧性能平缓下降）",
        "OOS 效率比 ≥ 0.5 且改进前后维持；至少 4/7 个市场环境不恶化",
        "负收益路径概率和 P5 最差回撤 不显著劣于基准",
        "6–12 个月模拟交易指标与 IS 预期行为一致",
    ],
    "适用改动类型": [
        "参数微调（止损乘数、窗口期、ADV 门槛等）",
        "新过滤逻辑（成交量过滤、市场环境过滤、新退出规则）",
        "新过滤逻辑（同上，作为 Layer 2 补充置信层）",
        "架构级改动（金字塔加仓、多空切换、组合级风控）",
    ],
    "本策略现状": [
        "✅ 已有（13 个扰动测试 + 热力图）",
        "✅ 已有（Walk-Forward 页 + 市场环境分析页）",
        "✅ 已有（蒙特卡洛模拟页）",
        "🔄 进行中（模拟交易监控页）",
    ],
}
...
st.info("""
**「高原 vs 孤峰」原则（Layer 1 最重要的判断标准）**

以 breakout_window 为例：N=200 在历史上 CAGR 最高。但这还不够——还要看 N=180、190、210、220 的结果：

- **高原（✅ 可接受）**：180→19%、190→19.5%、200→20%、210→19.5%、220→19% — 最优点两侧平滑下降，说明结论对参数值不敏感，是真实规律。
- **孤峰（❌ 危险）**：180→12%、190→15%、200→20%、210→14%、220→11% — 只有 200 特别好，高度依赖精确参数值，大概率是历史噪音的偶然拟合，实盘将失效。

**本策略参数敏感性分析**（见「参数敏感性分析」页面）已对 13 个参数做了扰动测试并计算稳定区间（CV < 0.10 = 高鲁棒），可用于 Layer 1 判断。
""")

st.markdown("---")

st.markdown("#### D. 接受改进的量化门槛")

st.markdown("""
有了验证流程，还需要明确"多少算好"。没有量化门槛，研究者容易在主观判断中放水——
看到 OOS CAGR 从 10.58% 提升到 10.68%，可能说"有改善，加入"。
但 0.10% 的 CAGR 变化完全在统计噪音范围内，不应作为接受改进的依据。
""")
...
_gate_data = {
    ...
    "说明": [
        "IS 无改善直接停止；IS 改善才进入 Layer 2",
        "低于 0.5pp 视为统计噪音，不值得增加复杂度",
        "只提升 CAGR 但 Sharpe 下降 = 仅增加了风险，无改善",
        "MaxDD 恶化超过 2pp 需权衡，超过 5pp 基本应拒绝",
        "只在 1–2 个特定环境有效 = 时期过拟合，非普适规律",
...
st.warning("""
⚠️ **上表数字为参考起点，需随策略演进校准。**
核心原则是：改进的统计显著性必须明显大于噪音水平，且不能以牺牲其他关键指标为代价。
若一项改动只有在"多指标里挑一两个好的来看"时才显得有效，通常是选择性报告而非真实改善。
""")

st.markdown("---")

st.markdown("#### E. 策略复杂度预算")

st.markdown("""
趋势跟踪策略最常见的失效模式不是"策略逻辑错了"，而是**"改进死亡"**：
每一次改进在 IS 上都有效，参数从 13 个增加到 25 个，
最终变成一个高度过拟合的黑盒，实盘一塌糊涂。

**建立复杂度预算，是预防这个结局的主动管控手段。**
""")

_col_e_a, _col_e_b = st.columns(2)
with _col_e_a:
    st.markdown(f"""
**当前策略参数预算：**
...
|------|-------|---------|
| 核心参数数量 | 13 个 | ≤ 18 个 |
| 过滤逻辑层数 | 3 层（价格/ADV/成交量） | ≤ 5 层 |
| 退出规则数量 | 2 种（止损 + 移动止盈） | ≤ 4 种 |

**超出上限时的处理原则：**
- 新参数进来，旧参数必须有一个出去（或合并）
- 不允许以"先加进去再看"的方式绕过预算
""")
with _col_e_b:
    st.markdown("""
...
| 最低贡献（OOS 验证后） | 示例 |
|---------------------|------|
| OOS CAGR 绝对提升 ≥ 0.5% | 10.58% → 11.1%+ |
| MaxDD 绝对改善 ≥ 1.5% pp | 19.3% → 17.8%- |
| OOS Sharpe 改善 ≥ 0.03 | 0.644 → 0.67+ |

**三选一即可，但必须通过 Walk-Forward OOS 验证，不能只看 IS。**

若新参数无法达到上述任一门槛，说明该改进带来的复杂度超过了其价值。
""")

st.info("""
**定期"复杂度审计"（建议每新增 3 项改进后执行一次）：**

1. 列出所有当前参数，评估每个参数的独立贡献（将其恢复默认值，看 OOS 结果变化多少）
2. 贡献不足门槛的参数，考虑固定为常量或删除
3. 优先测试"删减参数"而非"增加参数"——简单而稳健的策略比复杂而精确的策略更有实盘价值
""")

st.markdown("---")

st.markdown("#### F. 针对不同改动类型的具体操作规范")

with st.expander("🔧 参数微调（如调整止损乘数、ATR 窗口期、ADV 门槛）", expanded=True):
    st.markdown("""
**验证要求：Layer 1 即可（IS 回测 + 扰动）**

**判断标准（满足以下全部才接受改动）：**
- 主要指标（CAGR、Sharpe、MaxDD）至少两项改善，无一项显著恶化
- 扰动分析中，该参数的响应曲线在最优点附近**平滑**（高原形态，非孤峰），说明结论对参数值不敏感
- 交易笔数变化不超过 ±15%（过多/过少说明参数拉动了过滤器行为）

**反例（不应接受）：**
止损乘数从 2× 改为 2.03× 使 CAGR 从 10.58% 提升至 10.62%——
改善极小（0.04% 在噪音范围内），且 2.03 这个值没有任何经济学依据，纯属拟合。

**本策略已有的参数扰动见页面「参数敏感性分析」和「如何改进策略1.0」各节图表。**
""")

with st.expander("🆕 新增过滤逻辑（如 Choppiness Index、时间止损、Gap 过滤改 ATR 基准）"):
    st.markdown("""
**验证要求：Layer 1 + Layer 2（IS + Walk-Forward + 市场环境检验）**

**操作步骤：**
1. 写清假设（「引入 CI < 61.8 过滤可减少震荡市假突破，改善 Sharpe 和 MaxDD」）
2. 跑 IS 全历史回测，记录基线 vs 改进后所有指标
3. 用相同参数跑 Walk-Forward（推荐：5 年 IS + 2 年 OOS 滚动，或 2000–2015 IS + 2016–2026 OOS 锚定）
4. 检查各市场环境：改进在牛市、熊市、震荡市各环境是否均有效（至少 4/7 不恶化）
5. 计算 OOS 效率比，要求 ≥ 0.5，且 OOS 期间改进方向与 IS 一致

**关键检查**：改进是否只在某几个特定年份有效？
若 IS 的改善主要来自 2008 年和 2020 年（极端市场），OOS 在正常年份可能无效——
这说明改动是针对极端情形「专项优化」，而非普适性改善。
""")

with st.expander("🏗️ 架构级改动（金字塔加仓、多空切换对冲、Cluster Risk 组合级上限）"):
    st.markdown("""
**验证要求：Layer 1 + Layer 2 + Layer 3 + Layer 4（全部四层）**

**额外注意事项：**

- **改动复杂性等于自由度数量**：金字塔加仓引入「在多少 R 时加仓」「加仓比例」两个新参数，
  Walk-Forward 必须对这两个参数同时做 OOS 验证，而不是只对 IS 最优组合报告结果。

- **模拟交易是最后防线**：IS + WF + 蒙特卡洛都通过的改动，仍需在真实市场中跑 6–12 个月，
  因为回测无法模拟流动性冲击、订单填充细节、数据延迟等真实摩擦成本。

- **逐步推进**：先上线最简化版本（如只在单只标的 R 达到 3R 时允许金字塔加仓），
  验证行为符合预期后，再逐步扩展规则。一次上线完整复杂版本风险极高。
""")

st.markdown("#### G. 本策略已内置的防过拟合机制")
...
✅ **参数来自经济学直觉，而非最优化搜索**
- 2× ATR 止损：基于「给趋势呼吸空间」的直觉，不是网格搜索最优
- 200 日突破窗口：Donchian Channel 的经典设定，有 50 年学术文献支撑
- 1% NAV 风险/笔：经典趋势跟踪仓位管理，与 TF 基金行业惯例一致

✅ **扰动分析验证参数稳健性（高原形态）**
- 13 个核心参数的扰动结果（见本页各节）显示响应曲线大多呈高原形态，
  证明策略性能对参数值不过度敏感，不依赖精确的孤峰最优点

✅ **Walk-Forward + 市场环境 + 蒙特卡洛三重 OOS 防线**
- 本策略已完整实现 Research Pipeline 的 Layer 1–3 验证（见对应分析页面）
""")
with _col_g2:
    st.markdown("""
**仍需注意的风险：**
...
⚠️ **改进实验的累积效应**
随着本「改进方案」页面测试越来越多改进，消耗的历史数据「自由度」也在累积。
建议为每个重大改动专门预留一段时间段作为最终验证集（如 2024–2026 年），
在所有 IS 分析结束后，用这段时间做一次性最终验证。

⚠️ **模拟交易数据尚短**
当前模拟交易仅运行数周，尚无法提供 Layer 4 验证。需要至少 6 个月的模拟交易数据，
覆盖不同市场状态（强牛市、震荡市、小幅回调）后，结论才有足够信心。

⚠️ **相关性改进之间的交互效应**
多项改进同时上线时，效果可能相互增强或抵消，需要对「组合改进」再做一次整体 Walk-Forward，
而不是假设各项改进的效果可以线性叠加。
""")

st.info("""
**实用小结：改进一项功能前，问自己四个问题**

1. **我的假设是什么？** — 能用一句话说清楚经济学逻辑吗？
2. **如何证伪？** — 如果假设是错的，数据会呈现什么样子？
3. **OOS 怎么验证？** — 用哪段数据、哪个指标来判断改进是否泛化？
4. **复杂度预算还够吗？** — 这项改进是否值得占用一个参数名额？

只有四个问题都能清楚回答，才值得投入时间跑完整回测。
""")

st.markdown("""
<div class="info-box" style="background:#f0f7f0;border-left:4px solid #2ca02c;">
<strong>📌 当前阶段的改进优先级排序</strong><br><br>
对于一个已实现 10.6% CAGR、最大回撤 19.3% 的趋势跟踪策略，最大的风险不是「收益不够高」，
而是「为了继续提高收益，不断加入各种看起来有效的过滤器，最终过度优化」。<br><br>
<strong>建议优先级：</strong>
<ol style="margin:8px 0 0 16px">
<li><strong>鲁棒性</strong> — 扩大参数稳定区间，让策略在更宽的参数范围内表现一致</li>
<li><strong>OOS 一致性</strong> — 提高 Walk-Forward 效率比，让样本外表现更接近样本内</li>
<li><strong>降低回撤</strong> — 在不牺牲收益的前提下压缩 MaxDD</li>
<li><strong>提高收益</strong> — 在前三项充分满足后，再考虑进一步提升 CAGR</li>
</ol>
</div>
...
|:-----:|------|------------|:-----:|:-----:|:-------:|:-------:|:-------:|
| 🔴 **最高** | **入场条件** | **金字塔加仓**；Gap 改为 ATR 基准 | ✅改善 | ✅改善 | ✅改善 +4–5% | 🔻恶化 1–2% | ★★★ 高 |
| 🔴 **高** | **止损** | 止损执行价改为 min(stop, open[t+1]) | 中性 | ✅改善（最高） | 🔻恶化 −0.74pp | 中性 | ★ 低 |
| 🔴 **高** | **平价保护** | 浮盈达 1R 时止损移至略高于开仓价 | 中性 | 中性 | 中性 (+0.02pp) | ✅改善 −1.1pp | ★ 低 |
| 🔴 **高** | **分段移动止盈** | ≥10R 新增第四档 7×ATR | 中性 | 中性 | ✅改善 +1–2% | 中性 | ★ 低 |
| 🟠 **中高等** | **熊市做空对冲** | 熊市期（SPY<200MA）持有20%反向ETF | ✅改善 | 中性 | ✅改善 +2.4pp | ✅改善 | ★★ 中 |
| 🟠 **中高等** | **时间止损** | N 日无进展则主动退出 | 中性 | 中性 | ✅改善 | ✅改善 | ★★ 中 |
| 🟠 **中高等** | **Cluster Risk** | 相关性过滤升级为组合集中度上限 | 中性 | 中性 | 中性 | ✅改善 | ★★ 中 |
| 🟠 **中高等** | 市场环境过滤 | 增加 Choppiness Index 二层过滤 | ✅改善 | 中性 | 中性 | ✅改善 | ★★ 中 |
| 🟡 中等 | 标的过滤 | ADV 门槛通胀动态调整 | ✅改善 | 中性 | ✅改善 +0.5–1% | 中性 | ★★ 中 |
| 🟡 中等 | 执行假设 | 分层滑点；止损执行价（同止损节） | ✅改善 | ✅改善 | 中性 | 中性 | ★ 低 |
| 🟢 中低等 | 仓位管理 | 热度上限 heat_limit 降至 8% | 中性 | 中性 | 🔻恶化 −0.3% | ✅改善 | ★ 低 |
| 🟢 中低等 | 成交量过滤 | Walk-Forward 验证 1.5× 阈值稳健性 | ✅改善 | 中性 | 中性 | 中性 | ★★ 中 |

**图例：** ✅改善 / 中性 / 🔻恶化。优先级：🔴 高影响 → 🟠 中高等 → 🟡 中等 → 🟢 中低等。
""")

st.markdown("---")

# ── 二、利润来源：理解策略的盈利结构 ──────────────────────────────────────────────
st.subheader("三、利润来源分析：明白策略靠什么赚钱，才知道如何改进")
...
if _sl_d:
    ...
    _fig_sl.update_layout(
        title="止损乘数（stop_loss_multiplier）敏感性：CAGR、MaxDD、Sharpe",
...
策略1.0 止损 = 2×ATR，移动止盈初始倍数 = 3×ATR。当浮盈恰好达到 1.5R 时，移动止盈 = 最高价 − 3×ATR = (开仓价 + 1.5R) − 3×ATR = 开仓价。
...
# ── 九、仓位管理 ───────────────────────────────────────────────────────────────
st.subheader("八、熊市中加入 SPY 反向敞口（做空对冲）")
...
# ── 六、成交量过滤 ─────────────────────────────────────────────────────────────
st.subheader("九、时间止损：剔除「占座不赚钱」的仓位")

st.markdown("""
当前策略只有两种退出机制：**止损**（跌破 2×ATR 强制止损）和**移动止盈**（跌破移动止盈线）。
...
2. **平价保护 1R（Breakeven Protection）**（最大回撤改善）
   - 改动：在 exit.py 出场循环中增加 1 行判断
   - 效果：最大回撤改善约 −1.1 pp，CAGR 几乎不变（+0.02 pp），胜率轻微提升
   - 原理：浮盈达到 1R 时将止损上移至略高于开仓价，把"追涨到顶后回撤止损"变为"保本出场"
   - 详见：**如何用平价保护改善最大回撤** 页面

3. **大赢家第四档止盈：R_peak ≥ 10R → 7× ATR**（年化收益）
   - 改动：在 exit.py 中增加一档
