...
# 改进①：延伸回测至 1996 年
# ═══════════════════════════════════════════════════════════════════════════
st.subheader("改进① 延伸回测起始时间至 1996–1997 年")

st.markdown("""
**问题：** 当前回测从 2000-01-03 开始，但 Tiingo 数据已下载至 1996-01-01，这 4 年数据被完全闲置。

**影响：** 缺少了互联网泡沫 **上涨阶段**（1997–1999 是趋势跟踪策略历史上信号最密集的 3 年），
以及 2000 年泡沫**顶部到破裂**的完整样本。多 4 年等于增加约 15% 的样本量，
使 Walk-Forward 和蒙特卡洛分析的置信区间更窄、结论更可靠。
""")
...
st.markdown(f"""
**数据解读：**
- 1996 年可用标的 **{_n96} 只** → 1999 年增至 **{_n99} 只** → 2000 年 **{_n00} 只**
- 延伸期数据量有限（1996–1999 年仅 200–430 只），但科技泡沫时期价格趋势性极强，信号质量预计高于平均水平
- 实施成本：**接近零** —— 数据已存在，只需将回测引擎的 `backtest_start` 参数从 `2000-01-03` 改为 `1997-01-02`（1996 年数据用作 200 日高点的预热期）
""")

st.info("**建议**：将回测起始改为 1997-01-02（用 1996 全年作为 ATR/突破信号的预热窗口），预计可增加约 3,000–5,000 笔历史交易样本，统计置信度显著提升。")
