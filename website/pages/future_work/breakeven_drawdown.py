...
st.markdown("""
策略1.0 的参数设定为：**止损 = 2×ATR**（即 R = 2×ATR），**移动止盈倍数 = 3×ATR**。

当浮盈恰好为 1.5R 时，移动止盈位于：

> 移动止盈 = 最高价 − 3×ATR = (开仓价 + 1.5R) − 3×ATR = (开仓价 + 3×ATR) − 3×ATR = **开仓价**

即在浮盈达到 1.5R 时，**原始移动止盈已自然等于开仓价**，额外施加"平价保护 1.5R"完全没有保护作用。
同理，2R 阈值时移动止盈已高于开仓价格（= 开仓价 + ATR），平价保护 2R 同样无效。

| 浮盈阈值 | 此时原始移动止盈位置 | 平价保护效果 |
|---------|------------------|------------|
...
| 原始结果 | 数量 | 说明 |
|---------|------|------|
| 原始亏损（pnl ≤ 0） | **{_n_orig_loss} 笔** | 原始策略会亏损，BE 使其在稍高于开仓价的位置出场（净盈利 ≈ +$1） |
...
2. **预期价值为正**：{len(_ch)} 笔受影响交易中，{_n_helped} 笔改善（{_n_helped/len(_ch)*100:.0f}%），{_n_hurt} 笔变差（{_n_hurt/len(_ch)*100:.0f}%）。
   主要机制是将"追涨到顶后回撤至止损"的交易转化为"稍微盈利出场（净盈利 ≈ +$1）"。

3. **与现有移动止盈天然兼容**：平价保护 1R 仅在移动止盈原本低于开仓价时起作用（约浮盈 < 1.5R 阶段），
   不干扰后期趋势延续阶段（浮盈 ≥ 1.5R 后移动止盈已高于开仓价，两者等效）。

4. **实现简单**：在出场循环中，移动止盈更新后加一行判断即可：
   ```
...
其中约 **{len(_ch)/_trig_1r*100:.0f}%** 最终因平价保护提前出场（其余交易在触发保护后继续上涨，移动止盈自然高于开仓价，保护无需额外介入）。
""")
