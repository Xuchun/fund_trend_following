...
_fwd40_in_event = False
_fwd40_rows = []
for _fdt, _fdv in _dd.items():
    if not _fwd40_in_event and _fdv <= -40:
        _fep = float(_prices[_fdt])
        _frow = {
            "买入时间": _fdt.strftime("%Y-%m"),
            "买入价（$/oz）": f"{_fep:,.0f}",
        ...
        for _fy in [1, 2, 3, 5, 10, 20]:
            _ffd = _prices[_prices.index >= _fdt + pd.DateOffset(years=_fy)]
            _frow[f"{_fy}年后收益"] = f"{(_ffd.iloc[0] / _fep - 1):+.1%}" if len(_ffd) else "—"
        _fwd40_rows.append(_frow)
        _fwd40_in_event = True
    elif _fwd40_in_event and _fdv >= 0:
        _fwd40_in_event = False
if _fwd40_rows:
    st.dataframe(pd.DataFrame(_fwd40_rows), use_container_width=True, hide_index=True)
else:
...
st.markdown("**各次价格首次跌破 −30%（相对历史高点）时买入，后续收益：**")
_mo_fwd_in_event = False
_mo_fwd_rows = []
for _fdt, _fdv in _mo_dd.items():
    if not _mo_fwd_in_event and _fdv <= -30:
        ...
        _mo_fwd_in_event = True
    elif _mo_fwd_in_event and _fdv >= 0:
        _mo_fwd_in_event = False
if _mo_fwd_rows:
    st.dataframe(pd.DataFrame(_mo_fwd_rows), use_container_width=True, hide_index=True)
else:
    st.markdown("数据范围内无跌破 −30% 的时间点。")
...
st.markdown("**各次价格首次跌破 −50%（相对历史高点）时买入，后续收益：**")
_mo_fwd50_in_event = False
_mo_fwd50_rows = []
for _fdt, _fdv in _mo_dd.items():
    if not _mo_fwd50_in_event and _fdv <= -50:
        _fep = float(_mo_px[_fdt])
        _frow = {
            "买入时间": _fdt.strftime("%Y-%m"),
            "买入价（$/oz）": f"{_fep:,.0f}",
        ...
        for _fy in [1, 2, 3, 5, 10, 20]:
            _ffd = _mo_px[_mo_px.index >= _fdt + pd.DateOffset(years=_fy)]
            _frow[f"{_fy}年后收益"] = f"{(_ffd.iloc[0] / _fep - 1):+.1%}" if len(_ffd) else "—"
        _mo_fwd50_rows.append(_frow)
        _mo_fwd50_in_event = True
    elif _mo_fwd50_in_event and _fdv >= 0:
        _mo_fwd50_in_event = False
if _mo_fwd50_rows:
    st.dataframe(pd.DataFrame(_mo_fwd50_rows), use_container_width=True, hide_index=True)
else:
    st.markdown("数据范围内无跌破 −50% 的时间点。")
...
_yr_fwd_in_event = False
_yr_fwd_rows = []
for _fdt, _fdv in _yr_dd.items():
    if not _yr_fwd_in_event and _fdv <= -30:
        ...
        _yr_fwd_in_event = True
    elif _yr_fwd_in_event and _fdv >= 0:
        _yr_fwd_in_event = False
if _yr_fwd_rows:
...
st.markdown("**各次年均价格首次跌破 −40%（相对历史高点）时买入，后续收益：**")
_yr_fwd40_in_event = False
_yr_fwd40_rows = []
for _fdt, _fdv in _yr_dd.items():
    if not _yr_fwd40_in_event and _fdv <= -40:
        _fep = float(_yr_px[_fdt])
        _frow = {
            "买入年份": _fdt.year,
            "买入年均价（$/oz）": f"{_fep:,.2f}",
        ...
        for _fy in [1, 2, 3, 5, 10, 20]:
            _ffd = _yr_px[_yr_px.index >= _fdt + pd.DateOffset(years=_fy)]
            _frow[f"{_fy}年后收益"] = f"{(_ffd.iloc[0] / _fep - 1):+.1%}" if len(_ffd) else "—"
        _yr_fwd40_rows.append(_frow)
        _yr_fwd40_in_event = True
    elif _yr_fwd40_in_event and _fdv >= 0:
        _yr_fwd40_in_event = False
if _yr_fwd40_rows:
    st.dataframe(pd.DataFrame(_yr_fwd40_rows), use_container_width=True, hide_index=True)
else:
    st.markdown("数据范围内无跌破 −40% 的时间点。")
