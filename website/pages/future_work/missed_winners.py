...
# ── 角度三：信号强度（k 值）对比 ──────────────────────────────────────────────
st.markdown("#### 角度三：信号强度（k 值）对比——策略有没有优先选到更强的信号？")
st.markdown(
    "若错失信号的 k 值**高于**当天已执行信号的平均 k 值，说明策略在同天竞争中"
    "丢掉了更强的信号（优先队列改善空间）；反之则说明随机顺序下偶然选了弱的。"
)
st.info(
    "**k 值（k_strength）定义**：衡量突破当日收盘价超出过去 200 日最高价的幅度，"
    "以当日 ATR（20日真实波幅均值）为单位标准化：\n\n"
    "$$k = \\frac{\\text{收盘价} - \\text{200日最高价}}{\\text{ATR}_{20}}$$\n\n"
    "k 值越大，代表突破越强劲、越干脆。例如 k=2 表示收盘价超出前高 2 个 ATR，"
    "k=0.5 表示仅微弱突破。全量信号的 k 值范围约为 0.01–83，均值约 3.8。"
)
...
st.markdown(
    "高 k 值信号在均值和胜率上均优于低 k 值信号，为「优先执行高 k 值信号」提供了数据支撑。"
)
...
st.markdown("---")
# ── 建议一 ───────────────────────────────────────────────────────────────────
st.markdown("#### ✅ 建议一：按 k 强度建立信号优先级队列（低成本、立竿见影）")
st.markdown(
    f"""
**问题**：同天多个信号时，当前策略按数据处理顺序（近似随机）执行，
有时先执行了较弱的信号，资金耗尽后，更强的潜在大赢家反而被丢弃。

**数据支撑**：{n_miss50} 个错失 Top {TOP_N} 信号中，
**{n_pos_gap} 个（{n_pos_gap/max(n_miss50,1):.0%}）的 k 值高于当天已执行信号均值**，
说明调整同天顺序可以直接提升大赢家捕获率。

**实施方式**：回测引擎中，同一天的候选信号按 `k_strength` 降序排列后再逐一判断是否入场。
代码改动极小（一行排序逻辑），不改变任何参数和风险结构。

**预期效果**：在不增加仓位数、不改变持仓上限的前提下，在资金有限时优先选到最强突破信号，
减少"执行了弱信号而丢失强信号"的情况。
"""
)
...
st.markdown(
    """
**建议**：对 k 值极高（如 k > 5）的信号，在建议一（优先入场）的基础上，
...
summary = pd.DataFrame({
    ...
    "建议": [
        "按 k 强度建立信号优先级队列",
        "将 position_cap 从 5% 降至 3%",
        "对高 k 值信号给予更大仓位权重",
    ],
    "实施难度": ["低（改一行排序逻辑）", "中（需完整回测验证）", "高（需新逻辑 + 严格验证）"],
    "预期效果": ["改善同天竞争选择质量", "并发容量 +65%，捕获更多信号", "精准提升高质量突破暴露"],
    "最大回撤影响": ["中性", "应持平或降低", "需谨慎控制"],
})
st.dataframe(summary, use_container_width=True, hide_index=True)

st.success(
    "**推荐行动**：先在现有回测框架中将同天信号改为按 k_strength 降序排列（改动最小，风险最低），"
    "同时将 `position_cap` 从 0.05 改为 0.03 跑一次完整回测 + Walk-Forward，"
    "观察年化收益、最大回撤、交易次数的变化后，再决定是否引入高 k 加仓逻辑。"
)
