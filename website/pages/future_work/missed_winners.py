...
# ── 标题与概览 ────────────────────────────────────────────────────────────────
st.title("🏆 如何买到错失的大赢家")
st.markdown("从全量候选信号中找出被错失的高收益交易，深度解析原因，寻找改善年化收益的路径")
...
show_df(compare, width="stretch", hide_index=True)
...
with _c_bt2:
    ...
    st.plotly_chart(_fig_bt2, width="stretch")
...
with col_k3a:
    ...
    show_df(k_exec_perf, width="stretch")
with col_k3b:
    if len(k_exec_perf) > 0:
        ...
        st.plotly_chart(fig_k3, width="stretch")
...
summary = pd.DataFrame({
    ...
    "建议": [
        ...
        "对高 k 值信号给予更高初始仓位权重",
        "排序键从 breakout_strength 改为 k_strength",
    ],
    "实施难度": ["中（需完整回测验证）", "高（需新逻辑 + 严格验证）", "低（已回测验证）"],
    "预期效果": ["并发容量 +65%，捕获更多信号", "精准提升高质量突破暴露", "CAGR −1.19pp，MaxDD +3.03pp"],
    "最大回撤影响": ["应持平或降低", "需谨慎控制", "改善 3pp，但 CAGR 代价过高"],
    "当前状态": ["⏳ 待回测验证", "🔬 需严格 Walk-Forward 后引入", "🚫 回测显示净效果为负，维持原排序"],
})
show_df(summary, width="stretch", hide_index=True)

st.success(
    "**推荐行动**：唯一值得推进的改动是将 `position_cap` 从 0.05 改为 0.03，"
    "跑完整回测 + Walk-Forward，观察年化收益、最大回撤、交易次数的变化。"
    "排序键切换已经过回测验证，净效果为负，**维持现有 `breakout_strength` 排序**。"
    "position_cap 降至 3% 经回测确认有效后，再决定是否引入高 k 值加仓逻辑。"
)
