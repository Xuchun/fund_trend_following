...
st.title("如何改进策略有效性")
st.caption(
    '基于26年回测的「策略有效性分析」，深入研究Alpha压缩的根本原因，并从数据出发提出三条可落地的改进路径'
)
...
# ── Section 4: 改进路径二 ─────────────────────────────────────────────────────
st.subheader("四、改进路径二：熊市中加入 SPY 反向敞口（做空对冲）")
...
# ── Section 5: 改进路径三 ─────────────────────────────────────────────────────
st.subheader("五、改进路径三：让大赢家持仓更久（减少过早退出）")
...
st.markdown(
    ...
    "**实施建议：**\n\n"
    "- **第3/5级以上盈利单延迟调整追踪止损**：当盈利达到 3R 以上时，将追踪止损收紧速度放缓，"
...
# ── Section 6: 改进路径总结 ──────────────────────────────────────────────────
st.subheader("六、三条路径的综合对比与实施优先级")

_below  = trades[trades["spy_ma_ratio"] <= 0]
_hot    = trades[(trades["spy_ma_ratio"] >= 0.10) & (trades["spy_ma_ratio"] < 0.15)]
_summary_rows = [
    {
        "改进路径": "① 过热区间（10–15% 高于200MA）降仓",
        "实施难度": "低",
        "预期CAGR影响": "+0.1-0.2 pp（估算）",
        "预期Sharpe影响": "+小幅改善",
        "主要风险": "在过热区间少做会错过部分反弹行情",
        "数据支持": f"{len(_hot)} 笔过热区间交易平均R仅 {_hot['pnl_r_multiple'].mean():.3f}，是所有区间最低",
    },
    {
        "改进路径": "② 熊市期加入反向ETF对冲",
        "实施难度": "中",
        ...
        "数据支持": "回测显示 CAGR 提升 +2.4pp，Sharpe 显著改善",
    },
    {
        "改进路径": "③ 大赢家延迟收紧追踪止损",
        "实施难度": "中",
        ...
        "数据支持": f"3R+盈利单平均持仓155-170天，与持仓天数/R倍数正相关",
    },
]
...
st.markdown(
    "**实施优先建议：**\n\n"
    "1. **优先落地 ①（过热区间降仓）**：实施成本最低，在现有200MA过滤器基础上加一层距离判断即可，"
    "无需修改策略信号逻辑。该子区间（SPY 10–15% 高于200MA）历史上共有517笔交易，平均R趋近于零，"
    "减半仓位可显著降低风险暴露而几乎不损失期望收益。\n"
    "2. **中期测试 ②（熊市反向对冲）**：需要独立的 ETF 配置和风控规则，建议先在模拟账户验证后再实盘。"
    " 该路径是将策略从「纯多头趋势跟踪」升级为「多空双向策略」的关键一步，也是 **策略2.0** 的重要组成方向。\n"
    "3. **结合信号选择 ③（大赢家管理）**：与「如何选中更优开仓信号」和加仓金字塔机制联合设计，"
    "避免单独调整追踪止损参数导致整体策略失衡。"
)
