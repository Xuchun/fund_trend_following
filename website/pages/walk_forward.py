...
IS: 2000──────────────────────────2025  │  OOS: 2026（部分，截至 {meta.backtest_end}）
```
...
if wf_data:
    ...
    for w in windows:
        ...
        rows.append({
            ...
            "IS Sharpe":      f"{is_m.get('sharpe', 0):+.3f}",
...
""")
    ...
    with st.expander("▸ 展开：逐年解读与设计局限性"):
        st.markdown(f"""
...
但存在两点值得注意：
① 全部 IS 窗口均以 2000 年为起点，参数在互联网泡沫、金融危机和量化宽松的完整周期上被隐含优化，
