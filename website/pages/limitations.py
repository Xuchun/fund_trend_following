...
_avg_uw_str = f"≈{_avg_uw_yr:.1f} 年" if _avg_uw_yr is not None else "N/A（MC数据未加载）"
_p95_uw_str = f"≈{_p95_uw_yr:.1f} 年" if _p95_uw_yr is not None else "N/A"
...
| # | 局限性 | 严重程度 | 影响方向 | 量化估计 |
|---|-------|---------|---------|---------|
| 1 | **集中型牛市跑输大盘** | 🔴 高 | QE慢牛偏保守；AI牛市小幅跑赢 | {_qe_gap_str}；{_ai_gap_str} |
| 2 | **OOS 2022 亏损** | 🟡 中 | 隐藏环境风险 | 加息熊市单年 OOS {_w1_cagr_str} |
| 3 | **长期水下占用** | 🟡 中 | 流动性风险 | 平均水下 {_avg_uw_str}，P95 {_p95_uw_str} |
...
（互联网泡沫、GFC、2022 加息均跑赢 SPY），但**集中型**牛市跑输是核心代价
（QE 慢牛每年差约 **{abs(_qe_gap):.1f}%**，持续 11 年，复利差距显著）。
AI 驱动牛市（2023–2026）中因宽基趋势机会较多，策略 CAGR 高达 {_ai_cagr:.1f}%，小幅跑赢 SPY {abs(_ai_gap):.1f}%，
体现出不同牛市结构对策略的差异性影响。
