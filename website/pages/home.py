...
st.markdown(f"""
**推荐实盘的依据：**
1. Walk-Forward 4/5 窗口正收益、无过拟合，OOS CAGR {_wf_oos_cagr*100:+.1f}% 在实盘可实现范围内
2. 蒙特卡洛负收益概率 {_mc_neg_prob*100:.1f}%，策略1.0正期望高度确定
3. 策略逻辑透明，信号明确，无"看未来"的前视偏差
4. 在所有 5 个市场环境中均可存续，无单一环境下的致命弱点
5. 全部 13 项参数扰动测试完成，13/13 参数全范围正收益，参数鲁棒性充分验证

**实盘前必须完成的工作：**
- ⚠️ 使用**点对点历史成分股数据**（Compustat 或 CRSP）重新运行回测以消除幸存者偏差
- ⚠️ 验证 `trail_multiplier_r1 = 2.5×`、`stop_loss_multiplier = 2.5×` 在 OOS 数据上的改善是否可持续，再决定是否调整基准参数
