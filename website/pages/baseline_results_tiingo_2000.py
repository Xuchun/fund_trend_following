...
| **标的覆盖** | NYSE / NASDAQ / AMEX 全量历史股票 + {meta.universe_etfs} 只跨资产 ETF，含退市 / 被收购 / 破产标的 |
...
| 最低市值 | `min_market_cap_b` | **\$2B**（仅数据筛选阶段，引擎内部未二次过滤）|
...
st.markdown("---")
st.markdown('<div id="sec-deepdd"></div>', unsafe_allow_html=True)
...
st.markdown('<div id="sec-top20loss"></div>', unsafe_allow_html=True)
...
| 维度 | 数据 | 解读 |
|------|------|------|
| 退出方式 | {_t20_n_trailing}/20 笔（{_t20_n_trailing / 20 * 100:.0f}%）为移动止盈；另外 {20 - _t20_n_trailing} 笔（{_t20_non_trail_str}）为回测结束日仍持仓 | 大赢家几乎全靠"让利润奔跑"，极少被止损打出 |
...
""")
    ...
    for _ename, _ys, _ye in _eras:
        ...
        _era_alphas.append((_sc - _bc) * 100)
        _era_rows.append({
            "阶段":          _ename,
...
1. **Alpha 逐阶段下降**：早期（2000−2007）年化超额收益 **{_a1:+.1f} pp**，中期（2008−2015）降至 **{_a2:+.1f} pp**，近期（2016−2025）转为 **{_a3:+.1f} pp**（负值）。这主要由 QE 时代的超长单边牛市驱动，而非策略本身失效——在非牛市年份策略依然展现出优势。
    ...
    if _has_detail:
        ...
        if _n_heat == 0 and _n_corr == 0:
            _insight = (
                ...
                "热度超限和相关性过滤均未触发。真实原因在于 **position_cap 的压缩效应**："
                "策略目标每笔风险为 1% NAV，但仓位上限为 5% NAV。"
                "当公式算出的持仓金额超过上限时，头寸被截断，实际承担的风险远低于 1% NAV（历史平均约 0.24% NAV）。"
                "因此 20 笔仓位的合计热度仅约 20 × 0.24% = **4.8%**，远低于 10% 上限；"
                "而现金在持满 ~20 笔（20 × 5% = 100% NAV）时已耗尽。"
                "这说明当前参数下策略受**资金约束**而非**风险约束**限制。"
                "**但是恰恰由于实际承担的风险远低于 1% NAV（历史平均约 0.24% NAV），"
                "使得当最长连续亏损（34 笔）发生的时候，策略的回撤不是 34%+，而是 14.5%，"
                "这种被动降低单笔风险无意中大大降低了策略的回撤。**"
            )
        _reason_text = "\n".join(_reason_lines)
        st.markdown(
...
with _trade_hdr_col:
    st.markdown('<div id="sec-trades"></div>', unsafe_allow_html=True)
...
if _bt_kl_df is not None and not _bt_kl_df.empty:
    ...
    _bt_kl_n_actual = len(_bt_kl_df)

    _bt_vol_colors = [
        "#2ca02c" if float(_bt_kl_df["Close"].iloc[i]) >= float(_bt_kl_df["Open"].iloc[i])
        else "#d62728"
        for i in range(len(_bt_kl_df))
    ]
    _bt_kl_fig = _msp_bt_kl(rows=2, cols=1, shared_xaxes=True,
                              vertical_spacing=0.03, row_heights=[0.75, 0.25])
    _bt_kl_fig.add_trace(_go_bt_kl.Candlestick(
        x=_bt_kl_df.index,
        open=_bt_kl_df["Open"].values, high=_bt_kl_df["High"].values,
        low=_bt_kl_df["Low"].values,   close=_bt_kl_df["Close"].values,
    ...
    _bt_kl_fig.add_trace(_go_bt_kl.Bar(
        x=_bt_kl_df.index, y=_bt_kl_df["Volume"].values,
        marker_color=_bt_vol_colors, showlegend=False,
    ), row=2, col=1)

    _bt_kl_fig.add_hline(y=_bt_kl_ep, row=1, col=1,
        line_color="#1f77b4", line_dash="dash", line_width=1.5,
        annotation_text=f"买入价 ${_bt_kl_ep:.2f}",
        annotation_position="top left", annotation_font_color="#1f77b4")
    _bt_kl_fig.add_hline(y=_bt_kl_sp, row=1, col=1,
        line_color="#d62728", line_dash="dash", line_width=1.5,
        annotation_text=f"止损价 ${_bt_kl_sp:.2f}",
        annotation_position="bottom left", annotation_font_color="#d62728")
    _bt_kl_fig.add_vline(x=_bt_kl_edt.isoformat(),
        line_color="#1f77b4", line_dash="dot", line_width=1.5,
        annotation_text="开仓日", annotation_position="bottom left",
        annotation_font_color="#1f77b4")
    _bt_kl_fig.add_vline(x=_bt_kl_xdt.isoformat(),
        line_color="#ff7f0e", line_dash="dash", line_width=2)
    _bt_kl_fig.add_annotation(
        x=_bt_kl_xdt.isoformat(), xref="x", y=0.22, yref="paper",
        text="出场日", showarrow=False,
        font=dict(color="#ff7f0e", size=11), xanchor="right", yanchor="top")

    _bt_kl_x_end = _bt_kl_xdt + _pd_bt_kl.Timedelta(days=4)
    _bt_kl_fig.update_layout(
        title=f"{_bt_kl_tk}　已平仓（R={_bt_kl_r:+.2f}）　（共 {_bt_kl_n_actual} 根日K线）",
        height=520, template="plotly_white",
        margin=dict(l=60, r=20, t=50, b=20),
    )
    _bt_kl_fig.update_layout(xaxis_rangeslider_visible=False)
    _bt_kl_fig.update_yaxes(title_text="价格 ($)", row=1, col=1, showgrid=True, gridcolor="#eeeeee")
    ...
    _bt_kl_fig.add_annotation(
        ...
        text=(f"买入 {_bt_kl_edt.strftime('%Y-%m-%d')} @ ${_bt_kl_ep:.2f}  |  "
              f"出场 {_bt_kl_xdt.strftime('%Y-%m-%d')} @ ${_bt_kl_xp:.2f}  |  "
...
if _dl_tk_en:
    _dl_col_a, _dl_col_b = st.columns(2)
    with _dl_col_a:
        _dl_tk_multi = st.multiselect(
            "从列表选择标的（可多选）",
            _bt_kl_all_tickers, key="dl_flt_tk_multi",
        )
    with _dl_col_b:
        _dl_tk_text = st.text_input(
            "或手动输入标的（逗号分隔，如 AAPL, MSFT）",
            key="dl_flt_tk_text",
        )
    _dl_tk_set = set(t.upper() for t in _dl_tk_multi)
    for _dtx in _dl_tk_text.split(","):
        _dtx = _dtx.strip().upper()
        if _dtx:
            _dl_tk_set.add(_dtx)
else:
    _dl_tk_set = set()

# DL Filter 2 & 3: Entry / Exit date
_dl_d1, _dl_d2 = st.columns(2)
with _dl_d1:
    _dl_edt_en = st.checkbox("限制开仓最早日期（开仓日 ≥）", key="dl_flt_edt_en")
    if _dl_edt_en:
        _dl_edt_val = st.date_input(
            "开仓最早日期（该日期及之后开仓的交易才下载）",
            value=_bt_kl_all["entry_date"].min().date(),
            min_value=_bt_kl_all["entry_date"].min().date(),
            max_value=_bt_kl_all["entry_date"].max().date(),
            key="dl_flt_edt_val",
        ...
        _dl_edt_val = None
with _dl_d2:
    _dl_xdt_en = st.checkbox("限制平仓最晚日期（出场日 ≤）", key="dl_flt_xdt_en")
    if _dl_xdt_en:
        _dl_xdt_val = st.date_input(
            "平仓最晚日期（该日期及之前出场的交易才下载）",
            value=_bt_kl_all["exit_date"].max().date(),
            min_value=_bt_kl_all["exit_date"].min().date(),
            max_value=_bt_kl_all["exit_date"].max().date(),
            key="dl_flt_xdt_val",
...
# DL Filter 4: R value range
_dl_r1, _dl_r2 = st.columns(2)
with _dl_r1:
    _dl_rmin_en = st.checkbox("R 大于等于（R ≥ 某值）", key="dl_flt_rmin_en")
    if _dl_rmin_en:
        _dl_rmin = st.number_input(
            "R 最小值", value=0.0, step=0.5, format="%.2f", key="dl_flt_rmin_val",
        )
    else:
        _dl_rmin = None
with _dl_r2:
    _dl_rmax_en = st.checkbox("R 小于等于（R ≤ 某值）", key="dl_flt_rmax_en")
    if _dl_rmax_en:
        _dl_rmax = st.number_input(
            "R 最大值", value=0.0, step=0.5, format="%.2f", key="dl_flt_rmax_val",
        )
    else:
        _dl_rmax = None

# DL Filter 5: Holding days
_dl_h1, _dl_h2 = st.columns(2)
with _dl_h1:
    _dl_hmin_en = st.checkbox("持仓天数 大于等于（天 ≥）", key="dl_flt_hmin_en")
    if _dl_hmin_en:
        _dl_hmin = st.number_input(
            "持仓天数最小值", value=1, step=1, min_value=0, key="dl_flt_hmin_val",
        )
    else:
        _dl_hmin = None
with _dl_h2:
    _dl_hmax_en = st.checkbox("持仓天数 小于等于（天 ≤）", key="dl_flt_hmax_en")
    if _dl_hmax_en:
        _dl_hmax = st.number_input(
            "持仓天数最大值", value=100, step=1, min_value=0, key="dl_flt_hmax_val",
        )
    else:
        _dl_hmax = None

# 应用批量下载筛选条件
_bt_range_trades = _bt_kl_all.copy()
if _dl_tk_set:
    _bt_range_trades = _bt_range_trades[_bt_range_trades["ticker"].isin(_dl_tk_set)]
if _dl_edt_val is not None:
    _bt_range_trades = _bt_range_trades[_bt_range_trades["entry_date"].dt.date >= _dl_edt_val]
if _dl_xdt_val is not None:
    _bt_range_trades = _bt_range_trades[_bt_range_trades["exit_date"].dt.date <= _dl_xdt_val]
if _dl_rmin is not None:
    _bt_range_trades = _bt_range_trades[_bt_range_trades["pnl_r_multiple"] >= _dl_rmin]
if _dl_rmax is not None:
    _bt_range_trades = _bt_range_trades[_bt_range_trades["pnl_r_multiple"] <= _dl_rmax]
if _dl_hmin is not None:
    _bt_range_trades = _bt_range_trades[_bt_range_trades["holding_days"] >= _dl_hmin]
if _dl_hmax is not None:
    _bt_range_trades = _bt_range_trades[_bt_range_trades["holding_days"] <= _dl_hmax]
_bt_range_trades = _bt_range_trades.reset_index(drop=True)
