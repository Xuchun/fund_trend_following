...
st.markdown(
    "**可能的改进方法：**\n\n"
    "- **市场环境过滤（策略1.0已启用）**：策略1.0已内置 SPY 200日均线过滤（`regime_filter_enabled=True`），"
    "当 SPY 低于200日SMA时自动停止新开多仓。该34笔连亏发生于2010年5月—8月，彼时 SPY 虽在200MA附近波动，"
    "但仍处于均线上方，因此信号未被过滤。进一步可考虑加大过滤力度：当 SPY 近20日累计跌幅超过5%时也暂停开仓。\n"
