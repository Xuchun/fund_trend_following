...
| 操作 | 时间点 |
|------|--------|
| 突破信号计算 | t 日收盘后（t 日收盘价 vs t-N 至 t-1 日最高价的滚动最大值） |
| 成交量确认 | 比较 t 日成交量 vs mean(volume[t-60:t-1])（shift=1，无前视偏差） |
| 突破强度计算 | close[t] / rolling_high_N[t]，均基于 t 日收盘后已知数据 |
| 止损/移动止盈更新 | t 日收盘后 |
...
但它直到 <strong>2022 年才上市</strong>。如果将 SGOV 用于 2004–2026 全期回测，
