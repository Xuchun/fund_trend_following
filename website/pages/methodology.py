...
本策略选择 **2000-01-01** 作为历史回测起点（而非更早或更晚），以下详述理由。
""")
...
> 2026-06 行为整个回测期标的池的最终规模（含已退市的历史标的），与 2000–2005 年的快照逻辑不同：它包含回测全程任何时点曾经活跃的所有标的。
""")
...
③ **现金代理缺口从 2.5 年拉长到 4.5 年**：SHY 于 2002-07 上市。2000 起始已有 2.5 年无 SHY；退到 1998 起始变为 4.5 年（占总回测时长约 16%），现金处理假设的影响会系统性放大。
