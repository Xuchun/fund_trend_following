...
def _build_ps_md_report():
    import datetime as _dt_ps
    _now_ps = _dt_ps.datetime.now().strftime("%Y-%m-%d %H:%M")
    L: list[str] = []
    def _h(n, t): L.append(f"{'#'*n} {t}"); L.append("")
    def _row(*c): L.append("| " + " | ".join(str(x) for x in c) + " |")
    def _sep(n): L.append("|" + "|".join(["------"]*n) + "|")
    def _blank(): L.append("")
    def _hr(): L.append("---"); _blank()

    L.append(f"# {meta.display_name} — 参数敏感性分析")
    _blank()
    L.append(f"**回测期间：** {meta.backtest_start} → {meta.backtest_end}  ")
    L.append(f"**生成时间：** {_now_ps}")
    _blank(); _hr()

    _h(2, "分析框架")
    L.append("参数敏感性分析目的：验证策略1.0对参数选择的鲁棒性（Robustness）。")
    L.append("若策略在宽泛参数范围内均能盈利，说明结果不依赖特定参数的过拟合。")
    _blank()
    _row("参数","当前基准值","测试范围","分析目的","状态"); _sep(5)
    for _nm_ps, _lbl_ps, _base_ps, _rng_ps, _purp_ps in _PARAM_CFG:
        _stat_ps = "✅ 已完成" if (_PERTURB_DIR / f"{_nm_ps}.json").exists() else "🔜 待分析"
        _row(_lbl_ps, _base_ps, _rng_ps, _purp_ps, _stat_ps)
    _blank(); _hr()

    L.append("**IS/OOS 数据说明：** 本页所有扰动测试使用完整历史数据（"
             f"{meta.backtest_start} → {meta.backtest_end}），包含 Walk-Forward 标记为 OOS 的年份。"
             "策略参数在运行回测前已根据业界经验事先确定，并非通过观察 OOS 结果后反向挑选。"
             "扰动测试目的仅为验证鲁棒性，而非选择参数。参数热力图分析同样使用完整历史数据，保持一致性。")
    _blank(); _hr()

    # Per-parameter sections
    for _nm_ps, _lbl_ps, _, _, _ in _PARAM_CFG:
        _d_ps = _load(_nm_ps)
        if _d_ps is None:
            _h(2, f"⏳ {_lbl_ps}")
            L.append("扰动测试尚未完成。"); _blank(); _hr()
            continue
        _h(2, f"✅ {_lbl_ps}")
        _recs_ps = _d_ps["results"]
        _base_val_ps = _d_ps["baseline_value"]
        L.append(f"**来源：** perturbation/{_nm_ps}.json  |  "
                 f"回测期 {_d_ps.get('start','?')} → {_d_ps.get('end','?')}")
        _blank()
        _row("参数值","CAGR","MaxDD","Sharpe","Sortino","年换手率","均持仓","隐含年化成本"); _sep(8)
        _slippage_ps = p.get("slippage_bps", 10)
        _comm_ps     = p.get("commission_bps", 3)
        for _r_ps in _recs_ps:
            _v_ps  = _r_ps["param_value"]
            _tov_ps = _r_ps.get("annual_turnover", 0)
            if _nm_ps == "slippage_bps":
                _cost_ps = _tov_ps * (_v_ps + _comm_ps) * 2 / 100
            elif _nm_ps == "commission_bps":
                _cost_ps = _tov_ps * (_slippage_ps + _v_ps) * 2 / 100
            else:
                _cost_ps = _tov_ps * (_slippage_ps + _comm_ps) * 2 / 100
            _is_base = abs(_v_ps - _base_val_ps) < max(abs(_base_val_ps)*0.001, 0.001)
            _label_ps = f"**{_v_ps}（基准）**" if _is_base else str(_v_ps)
            _row(_label_ps,
                 f"{_r_ps.get('cagr',0)*100:+.2f}%",
                 f"{_r_ps.get('max_drawdown',0)*100:.1f}%",
                 f"{_r_ps.get('sharpe',0):+.3f}",
                 f"{_r_ps.get('sortino',0):+.3f}",
                 f"{_tov_ps:.2f}x",
                 f"{_r_ps.get('avg_holding_days',0):.0f}天",
                 f"≈{_cost_ps:.2f}%")
        _blank()
        _cv_cagr_ps  = _sensitivity_cv(_recs_ps, "cagr")
        _cv_shar_ps  = _sensitivity_cv(_recs_ps, "sharpe")
        _all_pos_ps  = _all_positive(_recs_ps)
        _lo_ps, _hi_ps = _stability_region(_recs_ps, "sharpe")
        _stab_str_ps = f"{_lo_ps} – {_hi_ps}" if _lo_ps is not None else "全范围稳定"
        L.append(f"- CAGR CV：{_cv_cagr_ps:.3f}（{_robustness_label(_cv_cagr_ps)}）  |  "
                 f"Sharpe CV：{_cv_shar_ps:.3f}（{_robustness_label(_cv_shar_ps)}）")
        L.append(f"- Sharpe 稳定区间（≥ 90% 峰值）：{_stab_str_ps}")
        L.append(f"- 全部参数值均实现正 CAGR：{'✅ 是' if _all_pos_ps else '⚠️ 否，存在负收益情景'}")
        _blank(); _hr()

    # Trade execution diagnostics
    _DIAG_PS = Path(__file__).resolve().parents[2] / "results" / "v1" / "diagnostics.json"
    _h(2, "交易执行诊断（Trade Execution Diagnostics）")
    if _DIAG_PS.exists():
        import json as _json_ps
        _diag_ps = _json_ps.loads(_DIAG_PS.read_text(encoding="utf-8"))
        _gs_ps = _diag_ps.get("gap_loss_stats", {})
        _eq_ps = _diag_ps.get("execution_quality", {})
        _sa_ps = _diag_ps.get("streak_analysis", {})
        L.append("#### A. Gap 止损分析")
        L.append(f"- 止损交易笔数：{_gs_ps.get('n_stop_trades',0):,}")
        L.append(f"- 实际平均止损损失：{_gs_ps.get('mean',0):.2f}R（理论 -1.00R）")
        L.append(f"- 在计划止损价内：{_gs_ps.get('pct_within_1r',0)*100:.1f}%  |  "
                 f"缺口 > 2R：{_gs_ps.get('pct_beyond_2r',0)*100:.1f}%  |  "
                 f"严重缺口 > 5R：{_gs_ps.get('pct_beyond_5r',0)*100:.1f}%")
        L.append(f"- 执行质量评估：**{_eq_ps.get('assessment','?').upper()}**  |  "
                 f"缺口影响：{_eq_ps.get('gap_impact_r',0):+.3f}R")
        _blank()
        L.append("#### B. 连续亏损序列分析")
        L.append(f"- 最长连续亏损：{_sa_ps.get('max_consecutive_losses',0)} 笔  |  "
                 f"平均序列长度：{_sa_ps.get('avg_streak_length',0):.2f}  |  "
                 f"亏损序列总数：{_sa_ps.get('total_streaks',0)}")
        _blank()
        _yearly_ps = _diag_ps.get("yearly_stats", [])
        if _yearly_ps:
            L.append("#### C. 逐年交易质量")
            _row("年份","交易笔数","胜率","平均R","最佳交易R","最差交易R","平均持仓天数"); _sep(7)
            for _yr_ps in _yearly_ps:
                _row(_yr_ps["year"], _yr_ps["n_trades"],
                     f"{_yr_ps['win_rate']*100:.1f}%",
                     f"{_yr_ps['avg_r']:+.3f}",
                     f"{_yr_ps['best_r']:+.3f}",
                     f"{_yr_ps['worst_r']:+.3f}",
                     f"{_yr_ps['avg_hold_days']:.0f}")
    else:
        L.append("诊断数据尚未生成。")
    _blank(); _hr()
    ...
    # Overall assessment
    _h(2, "综合评估")
    _completed_ps = [nm for nm, *_ in _PARAM_CFG if (_PERTURB_DIR / f"{nm}.json").exists()]
    _n_done_ps = len(_completed_ps)
    _n_total_ps = len(_PARAM_CFG)
    L.append(f"已完成：{_n_done_ps} / {_n_total_ps} 个参数（{_n_done_ps/_n_total_ps*100:.0f}%）")
    _blank()
    if _n_done_ps > 0:
        _all_cvs_ps = []
        _n_pos_ps = 0
        _row("参数","CAGR CV","Sharpe CV","Sharpe稳定区间","全正CAGR"); _sep(5)
        for _nm_ps2 in _completed_ps:
            _d_ps2 = _load(_nm_ps2)
            if _d_ps2 is None: continue
            _recs_ps2 = _d_ps2["results"]
            _cv_c2 = _sensitivity_cv(_recs_ps2, "cagr")
            _cv_s2 = _sensitivity_cv(_recs_ps2, "sharpe")
            _all_cvs_ps.append((_nm_ps2, _cv_c2, _cv_s2))
            _ap2 = _all_positive(_recs_ps2)
            if _ap2: _n_pos_ps += 1
            _lo2, _hi2 = _stability_region(_recs_ps2, "sharpe")
            _lbl2 = next((cfg[1] for cfg in _PARAM_CFG if cfg[0] == _nm_ps2), _nm_ps2)
            _row(_lbl2, f"{_cv_c2:.3f} {_robustness_label(_cv_c2)}",
                 f"{_cv_s2:.3f}", f"{_lo2}–{_hi2}" if _lo2 else "全范围", "✅" if _ap2 else "⚠️")
        _blank()
        L.append(f"**正收益覆盖率：{_n_pos_ps}/{_n_done_ps}（{_n_pos_ps/_n_done_ps*100:.0f}%）**")
    _blank()
    return "\n".join(L)
...
st.info(
    ...
    "**参数热力图分析同样使用完整历史数据（2004–2026）**，与扰动测试保持一致。"
    "两种分析的目的均为理解参数景观，而非为新策略版本选优——若未来需要为 Strategy 2.0 挑选参数，"
    "届时才须严格限制在 IS 截止日之前的数据。"
)

st.caption(
    "⚠️ **注意：各参数扰动测试中「基准值」的回测结果，可能与「Baseline 参数回测结果」页面的数值略有差异**"
    f"（约 ±0.3–0.5% CAGR）。"
    "原因：两者由独立的 Python 进程分批运行，每次启动都从 Wikipedia 实时抓取 S&P 900 成分股列表；"
    "若两次抓取之间有公司被加入/移出指数，universe 会略有不同，导致交易笔数和 CAGR 出现小幅偏差。"
    "这是使用实时成分股数据（而非固定快照）的已知局限，不影响扰动测试的鲁棒性结论。"
)
