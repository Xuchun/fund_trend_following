...
""")
    ...
    st.markdown(
        "每天自动运行的 Tiingo 同步程序只做一件事：检查现有标的的 `endDate`，更新 `is_active` 状态（即检测已退市的标的）。"
        "它**不会**发现新的、尚未进入 CSV 的标的——标的池是**半静态**的。\n\n"
        "举例：若某标的历史价格从未超过 &#36;10，当初生成 CSV 时已被排除。"
        "即使该标的半年后价格远超 &#36;10 且成交量也满足条件，每天的自动程序也不会把它加回来。"
