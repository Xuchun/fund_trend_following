...
""", unsafe_allow_html=True)
    ...
    st.markdown("**初始资金：** $200,000 USD（全新独立账户，调试期 2026-06-19 起，正式启动 2026-08-01）")
    ...
    if _m1_ok:
        ...
        st.markdown(
            "💡 **风险% NAV**：若该持仓被止损出场，预计亏损金额占当前总净值（NAV）的百分比。"
            "实际止损位取 止损价 与 移动止盈价 中较高者（即当前生效的出场线）。"
            "计算方式 = max(0, 入场价 − 实际止损位) × 持股数量 ÷ NAV。"
            "当移动止盈价高于入场价时（已锁定盈利），风险% NAV = 0。"
    ...
    st.markdown('<div id="sec-pt-10"></div>', unsafe_allow_html=True)
    ...
    if _sig_hist:
        st.markdown('<div id="sec-pt-11"></div>', unsafe_allow_html=True)
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    st.divider()
    st.markdown('<div id="sec-pt-14"></div>', unsafe_allow_html=True)
    st.subheader("十四、数据 & 标的池")
    ...
    st.markdown(
        ...
        "**CSV 的筛选口径：** CSV 由扫描本地 Tiingo 历史数据生成（约 22,000 个标的），"
        "保留**历史上至少有一天同时满足原始收盘价 > &#36;10 且 ADV₆₀ > &#36;20M** 的标的，共 4,477 个。"
        "从未达到这一门槛的标的（长期低价股、极低流动性股）完全不在 CSV 内。\n\n"
        "**层层过滤的完整路径：**\n\n"
        f"4,477 &nbsp; 历史上曾满足条件（含退市）  \n"
        f"&nbsp;&nbsp;&nbsp;└─ {_ds_active_all:,} &nbsp; `is_active=True`（现役）  \n"
        f"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└─ {int((_ds_df[(_ds_df['is_active']==True) & (_ds_df['eligible_days']>=252)]).shape[0]):,} &nbsp; `eligible_days ≥ 252`（有足够历史数据）  \n"
        f"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└─ {_ds_n_tiingo:,} &nbsp; 排除少量受限 ETF（VXX 等）  \n"
        f"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└─ **{_ds_n_total:,} &nbsp; 排除 YF 无法下载的 {_ds_n_tiingo - _ds_n_total} 只 → 每日实际扫描范围**\n\n"
    ...
    # ── 最后一个交易日原始收盘价 ──────────────────────────────────────────────
    st.markdown(
        f"**过去 201 个交易日的每日复权最高价**，和**最后一个交易日的复权收盘价**，和**最后一个交易日原始收盘价**（raw close，未复权），标的池共 **{len(_dbg_tickers):,}** 个标的。"
    )
    ...
    if st.button("准备数据（从 Yahoo Finance 下载过去 60 个交易日成交额）", key="debug_dolvol_btn"):
        if not _dbg_tickers:
            ...
            import datetime as _dbg_dt3
            import io as _dbg_io3
            import yfinance as _dbg_yf3

            _dbg3_end   = _dbg_dt3.date.today()
            _dbg3_start = _dbg3_end - _dbg_dt3.timedelta(days=120)  # 多取以确保满足60个交易日

            with st.spinner(f"正在从 Yahoo Finance 下载 {len(_dbg_tickers):,} 个标的的成交额数据…"):
                try:
                    _dbg3_raw = _dbg_yf3.download(
                        _dbg_tickers,
                        start=str(_dbg3_start),
                        end=str(_dbg3_end + _dbg_dt3.timedelta(days=1)),
                        auto_adjust=False,
                        progress=False,
                        threads=True,
                    )
    ...
    if st.session_state.get("debug_dolvol_csv"):
        st.download_button(
            label="点击下载 CSV（成交额）",
            data=st.session_state["debug_dolvol_csv"],
            file_name=st.session_state["debug_dolvol_filename"],
            mime="text/csv",
