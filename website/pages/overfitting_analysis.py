...
st.markdown("""
Block Bootstrap 月度分块重采样（块长≈21交易日），生成 1,000 条等长路径，验证策略绩效是否在随机打乱历史顺序后仍然稳健。
**过拟合症状**：若策略靠踏中少数关键月份的大行情，打乱后绝大多数路径会亏损——P5 CAGR 接近 0% 甚至负值。
""")
...
st.page_link("pages/monte_carlo.py", label="→ 蒙特卡洛模拟（完整分析：NAV 路径扇形图、回撤分布、水下时间）", icon="🎲")
...
**复杂度预算**是为了防止这个结局而主动建立的硬性上限规则，来源于「如何改进策略1.0 → E. 策略复杂度预算」（见页面底部链接）。
