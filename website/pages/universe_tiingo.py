...
# ── 标的池概览 ─────────────────────────────────────────────────────────────────
st.subheader("标的池概览（历史回测 2004-01-02 → 2026-06-12）")
...
with col_b:
    st.markdown("""
**过滤条件 2：ADV₆₀ > $20M（底池构建门槛）**
- ADV₆₀ = 60 日滚动平均日成交额 = close × volume（60 日均值），使用 shift(1) 无前视偏差
- 底池 CSV 使用 **$20M** 构建，保留更宽的历史标的；策略引擎运行时再按策略参数（Baseline: $60M）动态二次过滤
- 替代市值过滤（Tiingo 无历史流通股数据）：$20M 对应约 $4 亿+ 市值，$60M 对应约 $12 亿+市值
""")
