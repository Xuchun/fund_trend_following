...
为修正这一偏差，我们从 Tiingo 下载了 **NYSE / NASDAQ / AMEX 全量历史股票 + 84 只 ETF** 的日度价格数据
...
以下类型产品因具有**结构性负漂移**，不适合纯多头趋势跟踪策略，已从标的池中硬性排除：

| 排除类型 | 代表产品 | 排除原因 |
|---------|---------|---------|
| VIX 期货 ETF/ETN | VXX、UVXY、VIXY | 期货展期成本（Contango 损耗）导致长期必然亏损 |
| 3× 杠杆指数 | TQQQ、UPRO、SPXL、SQQQ | 每日复杠造成波动率拖拽，长期侵蚀本金 |
| 2× 杠杆/反向 | SSO、SDS、SH | 反向产品与多头趋势跟踪逻辑根本矛盾 |
| 3× 杠杆行业 | LABU/LABD、NUGT/DUST | 日内重置在震荡市造成系统性损耗 |

> 回测验证：VXX（曾混入标的池）3 笔交易合计亏损 **$380k**，是 ETF 子集整体亏损的主因。
> 排除后 ETF 标的池专注于真实趋势资产，避免负漂移产品拖累策略表现。
""")
...
| 项目 | 详情 |
|------|------|
| 下载范围 | NYSE / NASDAQ / AMEX 全量美股 + 84 只 ETF（排除波动率/杠杆产品后），共约 21,381 个 ticker |
