...
| **进入回测标的池（≥252 天）** | **{_universe_stocks:,}** | 股票候选池 {len(_eu_over252):,} 只（活跃 {_eu_over252["is_active"].sum():,} + 退市 {(~_eu_over252["is_active"]).sum():,}），减去 {_n_excl_in_cands} 只被结构性排除的 ETF（{_excl_in_cands_str}）= **{_universe_stocks:,}** 只最终股票策略池 |
