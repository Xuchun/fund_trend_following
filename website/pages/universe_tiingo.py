...
| **原始下载范围** | {_data_earliest} → {_data_latest}（约 {_download_yrs} 年） |
| **回测使用范围** | {_backtest_start} → {_data_latest}（约 {_backtest_yrs} 年，<span style="color:red;font-weight:bold">等待未来扩展至 1996</span>） |
| **覆盖标的** | NYSE / NASDAQ / AMEX 全量历史股票 + 87 只跨资产 ETF |
...
- 当前回测使用 2004 年起数据，跨越多次完整牛熊周期
- 日度复权价格，精确处理拆股/分红
- 85 只 ETF 覆盖跨资产（股票指数、债券、大宗商品、REITs、加密、国防、建筑）
""")
with _col_b:
    st.markdown("""
**局限**
...
# ── 标的池概览 ─────────────────────────────────────────────────────────────────
st.subheader(f"标的池概览（历史回测 2004-01-02 → {_data_latest}，标的过滤方法如下）")
...
为修正这一偏差，我们从 Tiingo 下载了 **NYSE / NASDAQ / AMEX 全量历史股票 + 87 只 ETF** 的日度价格数据
...
with col_b:
    st.markdown("""
**过滤条件 2：ADV₆₀ > $60M（Baseline 参数的过滤条件）**
- ADV₆₀ = 60 日滚动平均日成交额 = close × volume（60 日均值），使用 shift(1) 无前视偏差
- 替代市值过滤（Tiingo 无历史流通股数据）：$60M ADV 对应约 $12 亿+市值以上
- 引擎每日动态检查，确保标的在信号生成当日满足流动性要求
""")
...
- 具体实现：仅纳入在价格 > $10 且 ADV > 阈值的条件下，累计满足天数 ≥ 252 天的标的
""")
...
| 4 | **ADV 门槛静态** | 轻微 | $60M 门槛 2004 年偏紧（美股流动性较低），2024 年偏宽；动态门槛可更精确但实现复杂 |
""")
