...
| # | 缺陷 | 影响方向 | 说明 |
|---|------|---------|------|
| 1 | **无历史市值过滤** | 偏乐观 | Tiingo 无历史流通股数据，ADV > $50M 代替市值 > $10 亿，可能排除部分流动性刚达标的中盘标的 |
| 3 | **Tiingo 数据质量** | 中性 | 部分标的存在复权系数跳变（2,633 个）、价格尖峰（5,305 个），引擎的 `is_tradable` 标记已排除异常行 |
| 4 | **ADV 门槛静态** | 轻微 | $50M 门槛 2004 年偏紧（美股流动性较低），2024 年偏宽；动态门槛可更精确但实现复杂 |
""")
