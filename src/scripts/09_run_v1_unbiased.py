...
Runs two backtests back-to-back and compares them:
  Run A (baseline) : close > $10,  ADV > $60M,  eligible_days ≥ 252  → 2,943 tickers
...
Usage:
    ...
    python src/scripts/09_run_v1_unbiased.py --run a   # only the $60M run (Baseline)
...
DEFAULT_END   = "2026-06-15"   # matches Tiingo cache end; avoids incremental API calls
MIN_ELIGIBLE_DAYS = 252

BASE_PARAMS = StrategyParams(
    min_price               = 10.0,
    min_market_cap_b        = 2.0,
    min_adv_m               = 60.0,   # Baseline; overridden per run
    breakout_window         = 200,
    atr_period              = 20,
...
def main():
    ...
    if args.run in ("a", "both"):
        ...
        # Read existing meta to preserve fields set outside the backtest (etf_universe, etc.)
        _meta_path = out_a / "strategy_meta.json"
        _existing_meta: dict = {}
        ...
        _new_meta = {
            ...
            "universe_etfs":   len([t for t in universe if t in (strategy_tickers or []) and t in panel]),
            "params_anchor": {k: getattr(_p_a, k)
                              for k in vars(StrategyParams()) if not k.startswith("_")},
        }
        # Preserve etf_universe (hand-curated, not generated by backtest)
        for _keep in ("etf_universe",):
            if _keep in _existing_meta:
                _new_meta[_keep] = _existing_meta[_keep]
        _meta_path.write_text(json.dumps(_new_meta, ensure_ascii=False, indent=2))
