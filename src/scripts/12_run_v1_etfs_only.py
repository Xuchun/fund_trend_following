"""
Strategy 1.0 — ETFs-only backtest (2000-01-03 → 2026-06-15).
Same params as baseline; stocks excluded from strategy pool.
Outputs to results/v1_etfs_only_2000/.
"""
...
OUTPUT_DIR        = _root / "results" / "v1_etfs_only_2000"
...
def main():
    ...
    # ── ETFs only ──────────────────────────────────────────────────────────────
    universe = [t for t in all_tickers if t in etfs]
    logger.info("ETFs-only universe: %d tickers", len(universe))
    ...
    logger.info("Strategy pool (ETFs): %d tickers", len(strategy_tickers))
    ...
    print(f"\n{'='*65}")
    print(f"  Strategy 1.0 — ETFs ONLY backtest")
    print(f"  Pool: {len(strategy_tickers):,} ETFs  |  Period: {START} → {END}")
    print(f"{'='*65}\n")
